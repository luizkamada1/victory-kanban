...
export const KanbanCard = forwardRef<HTMLDivElement, KanbanCardProps>(function KanbanCard(
  { op, cor, dias, mostrarOficina, isDragging, isOverlay, emAndamento, dataAlvo, style, ...rest },
  ref
) {
  return (
    <div
    ...
    >
      ...
      <div style={isDragging ? estilos.conteudoInvisivel : undefined}>
        ...
        <div style={estilos.cardRodape}>
          ...
          {emAndamento && (
            <span style={estilos.cardEmAndamento} title="Em produção (ver tela exclusiva do setor)">
              🔧 Em andamento
            </span>
          )}
          {dataAlvo && (
            <span style={estilos.cardDataAlvo} title="Data alvo de conclusão">
              🎯 {formataPrevisao(dataAlvo)}
            </span>
          )}
        </div>
      </div>
